...
struct ContentView: View {
    ...
    cards[0].action = {
        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let window = windowScene.windows.first {
            let arQuickLookVC = ARQuickLookViewController(resourceName: "edgeOne2", allowScaling: true)
    ...
    cards[1].action = {
        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let window = windowScene.windows.first {
            let arQuickLookVC = ARQuickLookViewController(resourceName: "edgeTwo2", allowScaling: true) // Change resourceName to RuggedDevice6
            window.rootViewController?.present(arQuickLookVC, animated: true, completion: nil)
        }
    }
